...
for i in range(0, 1):
    ele_source = str(input("Enter the source node : "))
    source.append(ele_source) # adding the element

n = int(input("Enter number of destinations : "))
destinations=[]
for i in range(n):
    ele_dest=str(input())
    destinations.append(ele_dest)
for source in source:
    for destination in destinations:
        x, y=graph.shortest_path(source, destination)
        print(x,y)
